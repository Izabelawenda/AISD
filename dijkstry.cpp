...
{
    slistEl* next;
    int v, weight;                 // numer wêz³a docelowego i waga krawêdzi
};
...
int main()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (pw = graf[u]; pw; pw = pw->next)
            if (!QS[pw->v] && (d[pw->v] > d[u] + pw->weight))
            {
    ...
    {
        cout << i << ": ";
                                                                                            // Œcie¿kê przechodzimy od koñca ku pocz¹tkowi,   // Zapisuj¹c na stosie kolejne wierzcho³ki
